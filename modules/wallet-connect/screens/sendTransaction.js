...
import { View, Text, TouchableOpacity, StyleSheet, ToastAndroid, Image } from "react-native"; // @ts-ignore

import { RNCamera } from "react-native-camera"; // @ts-ignore
...
import ShimmerPlaceHolder from "react-native-shimmer-placeholder"; // @ts-ignore
...
const SendTransaction = props => {
  ...
  return <View style={styles.main}>
        ...
        <Fragment>
          {!qr ? <Fragment>
            <View style={styles.walletCard}>
              ...
              <View>
                <Text style={styles.balanceText}>{balance ? `${balance}` : <ShimmerPlaceHolder style={styles.WOeqnwes} width={50} height={15} LinearGradient={LinearGradient} />}</Text>
              </View>
            </View>
          ...
          </Fragment> : <QRCodeScanner onRead={onSuccess} flashMode={RNCamera.Constants.FlashMode.torch} topContent={<Text style={styles.centerText}>
                Scan QR code
      ...
      <View style={styles.bottomBtn}>
        {!qr && <Button onPress={() => StartTransaction()} disabled={!!(!receiver || !sender || !amount)}>Send</Button>}
      </View>
    </View>;
};
